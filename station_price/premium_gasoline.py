...
def by_premium_gasoline(target_df ):
    ...
    target_df_filltered = target_df[target_df['is_premium_gasoline_check']==1]
    i = 0
...
def upload_to_minio(df):
    ...
    table_name = "station_price/premium_gasoline"
...
if __name__ == "__main__":
    try:
        ...
        if not df.empty:
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {len(df)}개의 주유소의 고급 휘발유 가격을 발견. 업로드를 시작합니다")
            upload_to_minio(df)
        else:
